...
use crate::traits::Fp as FpTrait;
use crate::traits::fp::Fp2Helper;

/// GF(p^2) implementation.
#[derive(Clone, Copy, Debug)]
pub struct Fp2<Fp: FpTrait> {
    x0: Fp,
    x1: Fp,
}

impl<Fp: FpTrait> Fp2<Fp> {
    ...
    #[inline]
    // NOTE: this generic method is used only when:
    // 1. there is no implementation of sum_of_products and difference_of_products
    //    within the base field Fp
    // 2. for exceptional fields where additional subtractions mean that this method
    //    is faster.
    ...
    #[inline(always)]
    // NOTE: when the base field implemented Fp2Helper, it is likely that
    ...
    fn set_condneg(&mut self, ctl: u32) {
        self.x0.set_condneg(ctl);
        self.x1.set_condneg(ctl);
    }
    ...
    // TODO:
    // I can't do this with FpTrait as this is a const function and the trait doesnt want a const function?
...
impl<Fp: FpTrait + Fp2Helper> Fp2<Fp> {
    #[inline(always)]
    ...
    // TODO: how can i assure that this is overwritten?
    #[inline(always)]
    fn set_mul(&mut self, rhs: &Self) {
...
impl<Fp: FpTrait> ::std::fmt::Display for Fp2<Fp> {
    fn fmt(&self, f: &mut ::std::fmt::Formatter) -> ::std::fmt::Result {
        write!(f, "i*{} + {}", self.x1, self.x0)
    }
}

impl<Fp: FpTrait> ::core::ops::Add<Fp2<Fp>> for Fp2<Fp> {
    type Output = Fp2<Fp>;

    #[inline(always)]
    fn add(self, other: Fp2<Fp>) -> Fp2<Fp> {
        let mut r = self;
        r.set_add(&other);
        r
    }
...
impl<Fp: FpTrait> ::core::ops::AddAssign<Fp2<Fp>> for Fp2<Fp> {
    #[inline(always)]
    fn add_assign(&mut self, other: Fp2<Fp>) {
        self.set_add(&other);
    }
}
...
impl<Fp: FpTrait> ::core::ops::Div<Fp2<Fp>> for Fp2<Fp> {
    type Output = Fp2<Fp>;

    #[inline(always)]
    fn div(self, other: Fp2<Fp>) -> Fp2<Fp> {
        let mut r = self;
        r.set_div(&other);
        r
    }
...
impl<Fp: FpTrait> ::core::ops::DivAssign<Fp2<Fp>> for Fp2<Fp> {
    #[inline(always)]
    fn div_assign(&mut self, other: Fp2<Fp>) {
        self.set_div(&other);
    }
}
...
impl<Fp: FpTrait> ::core::ops::Mul<Fp2<Fp>> for Fp2<Fp> {
    type Output = Fp2<Fp>;

    #[inline(always)]
    fn mul(self, other: Fp2<Fp>) -> Fp2<Fp> {
        let mut r = self;
        r.set_mul(&other);
        r
    }
...
impl<Fp: FpTrait> ::core::ops::MulAssign<Fp2<Fp>> for Fp2<Fp> {
    #[inline(always)]
    fn mul_assign(&mut self, other: Fp2<Fp>) {
        self.set_mul(&other);
    }
}
...
impl<Fp: FpTrait> ::core::ops::Neg for Fp2<Fp> {
    type Output = Fp2<Fp>;
...
impl<Fp: FpTrait> ::core::ops::Sub<Fp2<Fp>> for Fp2<Fp> {
    type Output = Fp2<Fp>;

    #[inline(always)]
    fn sub(self, other: Fp2<Fp>) -> Fp2<Fp> {
        let mut r = self;
        r.set_sub(&other);
        r
    }
...
impl<Fp: FpTrait> ::core::ops::SubAssign<Fp2<Fp>> for Fp2<Fp> {
    #[inline(always)]
    fn sub_assign(&mut self, other: Fp2<Fp>) {
        self.set_sub(&other);
    }
}
...
impl<Fp: FpTrait> crate::traits::Fp for Fp2<Fp> {
    // Reexport constants for base field Trait
    const N: usize = <Fp>::N;
    const ENCODED_LENGTH: usize = 2 * <Fp>::ENCODED_LENGTH;
    const BIT_LENGTH: usize = <Fp>::BIT_LENGTH;
    const ZERO: Self = Fp2::ZERO;
    const ONE: Self = Fp2::ONE;
    const TWO: Self = Fp2::TWO;
    const THREE: Self = Fp2::THREE;
    const FOUR: Self = Fp2::FOUR;
    const MINUS_ONE: Self = Fp2::MINUS_ONE;
...
impl<Fp: FpTrait> crate::traits::Fp2 for Fp2<Fp> {
    type BaseField = Fp;
